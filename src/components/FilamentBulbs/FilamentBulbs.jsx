...
function FilamentBulbs({ category }) {
...
const navigate = useNavigate()
    ...
    const addCart = (product) => {
        ...
        const message = `Hi, I would like to purchase ${product.title} for Rs.${product.price} from your store. Here is the product image: [${product.imageUrl}](${product.imageUrl}). [Additional message if needed]`;
        ...
        <section className="text-gray-600 body-font">
            <div className="container px-5 py-8 md:py-16 mx-auto">
                <div className="lg:w-1/2 w-full mb-6 lg:mb-10">
                    <h1 className="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>Filament Bulbs</h1>
                    <div className="h-1 w-20 bg-[#fbae38] rounded"></div>
                </div>

                <div className="flex flex-wrap -m-4">
                    {product
                        .filter((obj) => obj.title.toLowerCase().includes(searchkey))
                        .filter((obj) => obj.category.toLowerCase() === category.toLowerCase()) // Filter by category
